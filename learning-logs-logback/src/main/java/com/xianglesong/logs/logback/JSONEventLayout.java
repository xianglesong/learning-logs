...
public class JSONEventLayout extends LayoutBase<ILoggingEvent> {
     ...
     *
     * <p> If you are embedding this layout within an <code>org.apache.log4j.net.SMTPAppender</code>
     * then make sure to set the <b>LocationInfo</b> option of that appender as well.
     */
     ...
     * @since 1.2.15
     */
    public void setMdc(final boolean flag) {
        mdc = flag;
    }
    ...
    public synchronized String doLayout(ILoggingEvent event) {
        ...
        appendKeyValue(buf, "system",
                system, null);
        buf.append(COMMA);
        appendKeyValue(buf, "application",
                application, null);
        buf.append(COMMA);
        appendKeyValue(buf, "version",
                version, null);
        buf.append(COMMA);

        appendKeyValue(buf, "uid", uid, mdc);
        buf.append(COMMA);
    ...
    /**
     * Location information dump with respect to call stack level. Some framework (Play) wraps the
     * original logging method, and dumping the location always log the file of the wrapper instead
     * of the actual caller. For PlayFramework, I use 2.
     */
